...
from lib.stg.courier_system_api_caller import CourierSystem
...
class DeliveryRepository:
    def insert_entry(self, conn: Connection, obj) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
                    INSERT INTO stg.deliverysystem_deliveries(object_id, object_value, delivery_ts, order_ts, update_ts)
    ...
    def get_max_delivery_ts(self, conn: Connection) -> TsObj:
        ...
        with conn.cursor(row_factory=class_row(TsObj)) as cur:
            cur.execute("SELECT max(delivery_ts) as max_ts FROM stg.deliverysystem_deliveries", {})
            res = cur.fetchone()
        return res
