...
class DeliveryDDSRepository:
    ...
    def insert_entry(self, conn: Connection, delivery: DeliveryObj) -> None:
        ...
        with conn.cursor(row_factory=class_row(IdObj)) as cur:
            ...
            cur.execute(
                """
                    select ord.id as order_id, cour.id as courier_id
                    from dds.dm_couriers cour, dds.dm_orders ord
                    where ord.order_id = %(order_key)s and cour.courier_id = %(courier_key)s
                """, 
                {
            ...
            cur.execute(
                """
                    INSERT INTO dds.dm_deliveries(delivery_id, delivery_ts, order_id, courier_id, address, rate, tip_sum)
...
class DeliveryLoader:
    WF_KEY = "dm_deliveries_stg_to_dds"
